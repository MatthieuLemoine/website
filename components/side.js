...
const Side = () => (
  <Container>
    ...
    <Group margin>{informations.map(info => <IconInfo key={info.icon} {...info} />)}</Group>
    <Group>
      <Expertise>{strings.expertise.title.toUpperCase()}</Expertise>
      {strings.expertise.items.map(item => (
        <ExpertiseItem key={item}>{item.toUpperCase()}</ExpertiseItem>
      ))}
    </Group>
  </Container>
);
